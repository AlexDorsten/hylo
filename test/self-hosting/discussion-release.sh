...
create_database() {
  ...
  docker compose exec -T db psql -X -v ON_ERROR_STOP=1 -U postgres -d "$1" -c 'CREATE EXTENSION postgis; CREATE EXTENSION pg_stat_statements; CREATE EXTENSION "uuid-ossp";'
}
restore_database() {
  # Restore as the unprivileged application owner; extensions are preinstalled.
  docker compose exec -T db pg_restore -U postgres --role=hylo --no-owner --no-privileges --no-comments --exit-on-error -d "$1" < "$2"
}
rehearse() {
...
docker compose exec -T db pg_dump -U postgres -d hylo -Fc --no-owner --no-privileges > "$backup_dir/fresh.dump"
...
docker compose exec -T db pg_dump -U postgres -d hylo_ci_upgrade -Fc --no-owner --no-privileges > "$backup_dir/before.dump"
...
rehearse hylo_ci_upgrade yarn workspace backend migrate
docker compose exec -T db pg_dump -U postgres -d hylo_ci_upgrade -Fc --no-owner --no-privileges > "$backup_dir/after.dump"
